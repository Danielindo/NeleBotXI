...
function urlConv($s) {
	return new Tag('<a href="' . htmlspecialchars($s) . '">', '</a>');
}

$ENTITIES_TO_TAG = array(
	'italic' => new Tag('<i>', '</i>'),
	'bold' => new Tag('<b>', '</b>'),
	'code' => new Tag('<pre>', '</pre>'),
	'strikethrough' => new Tag('<s>', '</s>'),
	'spoiler' => new Tag('<span class="tg-spoiler">', '</span>'),
	'underline' => new Tag('<u>', '</u>'),
	'pre' => new Tag('<pre>', '</pre>'),
	'phone' => null,
	'hashtag' => null,
	'url' => 'urlConv',
	'text_link' => function ($e, $s) {
		return new Tag('<a href="' . htmlspecialchars($e['url']) . '">', '</a>');
	},
	'text_mention' => function ($e, $s) {
		return new Tag('<a href="tg://user?id=' . htmlspecialchars($e['user']['id']) . '">', '</a>');
	},
);
...
function messageToHtmlConverter($message, $entities) {
	global $ENTITIES_TO_TAG;

	if (empty($entities) && strpos($message, "\n") === false) {
		return $message;
	}

	$html = '';
	$message_utf8 = $message;
	$message_b16 = iconv('UTF-8', 'UTF-16LE', $message_utf8);

	$positions = array();
	$positions[0] = new PositionChange();
	$positions[strlen($message_b16)] = new PositionChange();

	if (!empty($entities)) {
		prepareEntityPositionsUtf16le($message_b16, $entities, $positions);
	}

	prerapeBrPositions($message_utf8, $message_b16, $positions);

	$separations_points = array_keys($positions);
	sort($separations_points);

	$opened_tags = array();

	foreach ($separations_points as $pos => $index) {
		if ($index == strlen($message_b16)) {
			$next_index = null;
		} else {
			$next_index = $separations_points[$pos + 1];
		}

		$unchanged_part_utf8 = mb_convert_encoding(substr($message_b16, $index, $next_index - $index), 'UTF-8', 'UTF-16LE');
		$unchanged_part = str_replace("\n", "\n", htmlspecialchars_decode($unchanged_part_utf8));

		foreach ($positions[$index]->to_close as $t) {
			array_pop($opened_tags);
			$html .= $t->closing;
		}

		if ($positions[$index]->br) {
			if (in_array($ENTITIES_TO_TAG['code'], $opened_tags)) {
				// If inside a code block, ignore the new line
			} else {
				// Use \n to represent a newline within a paragraph
				$html .= "";
			}
		}

		foreach ($positions[$index]->to_open as $t) {
			$opened_tags[] = $t;
			$html .= $t->opening;
		}
...
function prepareEntityPositionsUtf16le($message_b16, $entities, &$positions) {
	global $ENTITIES_TO_TAG;

	foreach ($entities as $e) {
		$start = $e['offset'] * 2;
		$end = ($e['offset'] + $e['length']) * 2;
		ensurePositionExists($start, $positions);
		ensurePositionExists($end, $positions);
		$tag = $ENTITIES_TO_TAG[$e['type']];
		if ($tag) {
			if (is_callable($tag)) {
				$txt_bytes = substr($message_b16, $start, $end - $start);
				$txt = mb_convert_encoding($txt_bytes, 'UTF-8', 'UTF-16LE');
				$tag = call_user_func($tag, $e, $txt);
			}
			$positions[$start]->to_open[] = $tag;
			array_unshift($positions[$end]->to_close, $tag);
		}
	}
}

function prerapeBrPositions($message, $message_b16, &$positions) {
	for ($i = 0; $i < strlen($message); $i++) {
		if ($message[$i] == "\n") {
			$i = strlen(mb_convert_encoding(substr($message, 0, $i), 'UTF-16LE', 'UTF-8'));
			ensurePositionExists($i, $positions);
			$positions[$i]->br = true;
		}
	}
}

function ensurePositionExists($i, &$positions) {
	if (!isset($positions[$i])) {
		$positions[$i] = new PositionChange();
	}
}
...
class Variables
{
	...
	public function varPreCheckoutQuery ($Query) {
		if (empty($Query)) return;
		$this->preck_id = $Query['id'];
		$this->preck_currency = $Query['currency'];
		$this->preck_amount = $Query['total_amount'];
		$this->preck_payload = $Query['invoice_payload'];
		$this->varUser($Query['from']);
	}

	public function varShippingQuery ($Shipping) {
		if (empty($Shipping)) return;
		$this->varMessage($Shipping['message']);
	}
